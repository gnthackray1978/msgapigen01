    ...
    public class DescendantGraphBuilder 
    {
        ...
        public List<DescendantNode> GenerateDescendantGraph(int personId)
        {
           ...
           // var tree = _azureDbContext.TreeRecord
            ...
            foreach(var person in missingFathers)
            {
                ...
                _relationships.Add(new Relationships(){BrideId = _persons[fidx].MotherId.Value, GroomId = _persons[fidx].FatherId.Value });
            ...
            foreach (var person in missingMothers)
            {
                ...
                _relationships.Add(new Relationships() { BrideId = _persons[midx].FatherId.Value,
                    GroomId = _persons[midx].MotherId.Value });
            ...
            if(child!=null)
               spouseList = makeSpouse(child.PersonId.Value, 0);
            else
                return flattenedResults;
        ...
        private void fillChildGenerations(int personId, ref List<List<DescendantNode>> destination, ref int currentGeneration)
        {
            ...
            foreach (var child in children)
            {
                ...
                var spouseList = makeSpouse(child.PersonId.Value, currentGeneration);
        ...
        private List<DescendantNode> makeSpouse(int personId, int generationNumber)
        {
            ...
            var persons = _persons.Where(w => spouseIdList.Contains(w.PersonId.Value));
