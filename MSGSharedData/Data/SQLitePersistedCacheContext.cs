...
namespace FTMContextNet.Data
{
    public partial class SQLitePersistedCacheContext : DbContext, IPersistedCacheContext
    {
        ...
        public int BulkInsertFTMPersonView(int importId,int userId, List<FTMPersonView> ftmPersonViews)
        {
            ...
            var command = connection.CreateCommand();
            command.CommandText = "INSERT INTO FTMPersonView(Id, FirstName, Surname, BirthFrom, BirthTo, Location, BirthLat, BirthLong, AltLocationDesc, AltLocation, AltLat, AltLong, Origin, PersonId, LinkedLocations, FatherId, MotherId, LocationsCached, ImportId, DirectAncestor, RootPerson,LinkNode, UserId)" +
                                  " VALUES ($Id, $FirstName, $Surname, $BirthFrom, $BirthTo, $Location, $BirthLat, $BirthLong, $AltLocationDesc, $AltLocation, $AltLat, $AltLong, $Origin, $PersonId, $LinkedLocations, $FatherId, $MotherId, $LocationsCached, $ImportId, $DirectAncestor, $RootPerson,$LinkNode, $UserId);";
            ...
            foreach (var row in ftmPersonViews)
            {
                ...
                command.Parameters["$BirthFrom"].Value = row.YearStart;
                command.Parameters["$BirthTo"].Value = row.YearEnd;
                command.Parameters["$Location"].Value = row.Location;
                command.Parameters["$BirthLat"].Value = row.Lat;
                command.Parameters["$BirthLong"].Value = row.Lng;
